...

{
    ...
    h_weights = fs_->make<TH1F>("weights", "weights", 200, 0., 1.);
...
PUPPILeptonIsoProducer_Matthias::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    std::cout<<" !!!!!!!!!! "<<pfCandidates->size()<<std::endl;
	...
	double charged = 0, neutral = 0, photons  = 0;
	    ...
	    //std::cout<<" !!!!!!!!!! "<<pf.pt()<<" "<<pf.pdgId()<<std::endl;
	    //if (abs(pf.pdgId()==13))continue;
		    ...
		    {
		      //std::cout<<"inside!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! "<< leptons->size()<<" "<<footprint.size()<<std::endl;
		...
		if (pf.charge() == 0)
                {
   		    if (pf.pdgId()!=22 && pf.pdgId()!=130)
		      h_pfid->Fill(pf.pdgId());
		...
		else
		{
		    if (weight==1)
		    {
			//std::cout<<"meta "<< leptons->size()<<" "<<footprint.size()<<std::endl;
